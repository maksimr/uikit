...
export function timeAgoFromMs(ms) {
  const { sec, min, hr, day, month, year } = msToDateParts(ms);
 ...
 * @returns {string}
 */
export function microTimeFromMs(ms) {
  ms = Math.abs(ms);
  const { min, hr, day, year } = msToDateParts(ms);
...
/**
 * @param {number} ms 
 * @returns {{sec: number, min: number, hr: number, day: number, month: number, year: number}}
 */

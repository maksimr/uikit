...
import { countWeekendsBetween, getTimeZoneOffset, isLeapYear, isSameDay, weekDay } from './date-util';
...
describe('date-util', function() {
  ...
  describe('countWeekendsBetween', function() {
    ...
    it('should count weekends for date with hours & minutes', function() {
      ...
      const saturday = weekDay(DAY_OF_WEEK.SATURDAY, { minutes: 5 });
      expect(
        countWeekendsBetween(friday, saturday)
      ).toEqual(1);
    });
  ...
  describe('isSameDay', function() {
    ...
    it('should return false if passed dates are different days', function() {
      const day1 = new Date();
      day1.setDate(1);
      const day2 = new Date(day1.valueOf());
      day2.setDate(2);
  ...
  describe('getTimeZoneOffset', function() {
    it('should return time zone offset in minutes using browser database', function() {
      expect(getTimeZoneOffset(new Date(0), 'Europe/London')).toEqual(-60);
      expect(getTimeZoneOffset(new Date(0), 'America/Los_Angeles')).toEqual(480);
      expect(getTimeZoneOffset(new Date(0), 'Asia/Kolkata')).toEqual(-330);
    });
  });
});
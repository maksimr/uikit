...
/**
 * @enum
 * @type {{TOP_LEFT: number, RIGHT_BOTTOM: number, BOTTOM_CENTER: number, LEFT_CENTER: number, BOTTOM_RIGHT: number, TOP_CENTER: number, RIGHT_TOP: number, TOP_RIGHT: number, LEFT_BOTTOM: number, LEFT_TOP: number, RIGHT_CENTER: number, BOTTOM_LEFT: number}}
 */
...
export function Popup({
  ...
  parentNode = parentNode || popupConfig?.parentNode || anchorNode?.parentNode;
  ...
  return createPortal(
    <div data-role='popup' ref={setPopupNode} style={style} {...props}>
      {children}
    </div>,
    parentNode
  );
...
/**
 * @param {{parentNode: HTMLElement}} value
 * @param {JSX.Element} children
 * @returns {JSX.Element}
 * @constructor
 */
 ...
 * @param {HTMLElement} parentNode
 * @returns {{top: number, left: number, position: string}|{position: string}}
 */

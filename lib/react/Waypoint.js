...
export function Waypoint({ children, topOffset = 0, bottomOffset = 0, horizontal = false, onEnter, onLeave, scrollableAncestorElement }) {
  const refElement = useRef(/**@type {HTMLElement|null}*/(null));
  ...
  useEffect(() => {
    ...
    let _previousPosition = /**@type {Position|null}*/(null);
    ...
    const scrollableAncestor = (/**@type {HTMLElement|null}*/(scrollableAncestorElement ?? findScrollableContainer(element))) ?? window;
    const removeEventListeners = [
      addEventListener(scrollableAncestor, 'scroll', onScrollOrResize, { passive: true }),
      addEventListener(window, 'resize', onScrollOrResize, { passive: true })
    ];

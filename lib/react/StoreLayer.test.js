...
describe('StoreLayer', function() {
  ...
  let store;
  beforeEach(function() {
    store = new Store({ counter: 0 }, (state, fn) => {
  ...
  it('should redraw only components which store state has changed', function() {
     ...
     */
    const App = jasmine.createSpy().and.callFake(() => <div><A /><B /></div>);
    /**@type {import('react').FunctionComponent & jasmine.Spy<*>}*/
    const A = jasmine.createSpy().and.callFake(() => {
      useStore(qa);
      return <div />;
    });
    /**@type {import('react').FunctionComponent & jasmine.Spy<*>}*/
    const B = jasmine.createSpy().and.callFake(() => {
      useStore(qb);
      return <div />;
    });

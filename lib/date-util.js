...
export function getTimeZoneOffset(date, timezone) {
  // 'longOffset' always formats as GMT±HH:MM, unlike 'short' which may return
  // an abbreviation (BST, CET, GMT) that hides the real offset
  const str = date.toLocaleString('en-GB', {
    timeZone: timezone,
    timeZoneName: 'longOffset'
  });
  const match = str.match(/GMT([-+])(\d\d):(\d\d)$/);
  if (match) {
    const offset =
      (match[1] === '-' ? -1 : 1) *

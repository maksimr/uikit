...
describe('Store', function() {
  ...
  it('should not invoke listener immediately', function() {
    const onStateChange = jest.fn();
    store.watch(onStateChange, false);
  ...
  it('should not call listeners which was added during calling process', () => {
    ...
    const onStateChangeA = jest.fn((state) => {
      if (state === 1) {
        store.watch(onStateChangeB, false);
      }
    });

    store.watch(onStateChangeA, false);
    store.swap(() => (1));
  ...
  it('should call listeners which go after removed listener', function() {
    ...
    store.watch(onStateChangeBeforeRemovedListener, false);
    const removeListener = store.watch(() => removeListener(), false);
    store.watch(onStateChangeAfterRemovedListener, false);
  ...
  it('should correctly handle the case when user trigger swap in listener and remove other listeners in nested change', function() {
    ...
    store.watch(onStateChangeBeforeRemovedListener, false);
    const removeListener = store.watch(onStateChangeRemovedListenerAndTriggerSwap, false);
    store.watch(onStateChangeAfterRemovedListener, false);

...
describe('Store', function() {
  ...
  it('should not invoke listener immediately by default', function() {
    const onStateChange = jasmine.createSpy();
  ...
  it('should not call listeners which was added during calling process', () => {
    const onStateChangeB = jasmine.createSpy();
    const onStateChangeA = jasmine.createSpy().and.callFake((state) => {
      if (state === 1) {
  ...
  it('should call listeners which go after removed listener', function() {
    const onStateChangeBeforeRemovedListener = jasmine.createSpy();
    const onStateChangeAfterRemovedListener = jasmine.createSpy();
  ...
  it('should correctly handle the case when user trigger swap in listener and remove other listeners in nested change', function() {
    const onStateChangeBeforeRemovedListener = jasmine.createSpy();
    const onStateChangeAfterRemovedListener = jasmine.createSpy();
    const onStateChangeRemovedListenerAndTriggerSwap = jasmine.createSpy().and.callFake((state) => {

import { createStore, createCursor } from './store';
...
describe('Store', function() {
  ...
  describe('cursor', function() {
    it('should create cursor on the store and trigger watcher only if cursor state has changed', function() {
      store.swap(() => ({ foo: {} }));
      const onStateChange = createSpy();
      const { store: fooStore } = createCursor(store, (state) => state?.foo);
    ...
    it('should remove cursor', function() {
      store.swap(() => ({ foo: {} }));
      const onStateChange = createSpy();
      const { store: fooStore, destroy } = createCursor(store, (state) => state?.foo);
      fooStore.watch(onStateChange);
      destroy();

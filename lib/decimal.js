...
export function add(decimalX, decimalY) {
  ...
  const bigIntResult = bigIntY + bigIntX;
  return {
    sign: bigIntResult < 0 ? -1 : 1,
    mantissa: bigIntResult.toString().replace(/^-/, ''),
    exponent: Math.min(decimalX.exponent, decimalY.exponent)
  };
}
...
export function multiply(
  ...
  return {
    sign: bigIntResult < 0 ? -1 : 1,
    mantissa: bigIntResult.toString().replace(/^-/, ''),
    exponent: decimalX.exponent + decimalY.exponent
  };
}

export function decimal(/**@type {string}*/str) {
  return parse(str);
}
...
export function stringify(/**@type {BigDecimal | null}*/decimal) {
  ...
  const zeroPad = '0'.repeat(Math.max(-dx, 0));
  const integerPart = decimal.mantissa.slice(0, idx) || '0';
  const fractionPart = decimal.mantissa.slice(idx).replace(/0+$/, '') || '';
  return `${decimal.sign === -1 ? '-' : ''}${integerPart}${fractionPart ? '.' + zeroPad + fractionPart : ''}`;
}

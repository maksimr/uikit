 ...
 * @param {(state: State) => (QueryState|void)} query
 * @returns {{store: Store<ReturnType<query>>, destroy: () => void}}
 */
export function cursor(rootStore, query) {
  const ref = { isLocked: false };
  const store = new Store(query(rootStore.value), (_, fn) => {
    ref.isLocked = true;
  ...
  const destroy = rootStore.watch((rootState) => {
    if (!ref.isLocked && query(rootState) !== store.value) {
      store.swap((state) => state);
    }
  });

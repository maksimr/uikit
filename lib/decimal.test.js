...
describe('decimal', () => {
  ...
  it('should compare two decimals', () => {
    expect(BigDecimal.from(0.1).compareTo(0.2)).toEqual(-1);
    expect(BigDecimal.from(0.2).compareTo(0.1)).toEqual(1);
    expect(BigDecimal.from(0.2).compareTo(0.2)).toEqual(0);

    expect(BigDecimal.from(0.2).add(0.1).compareTo(0.3)).toEqual(0);
  });
});
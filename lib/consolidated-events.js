 ...
 *  by consolidating the same events into one
...
export function addEventListener(element, type, fn, options) {
  ...
  const eventKey = type + eventOptionsKey(options);
  ...
  if (eventListeners.length === 1) {
    attachEventListenerToNode(element, type, options);
  }
 ...
 */
function attachEventListenerToNode(element, type, options) {
  const eventKey = type + eventOptionsKey(options);
  const events = elementToEvents.get(element);
  const eventDescription = events[eventKey] = events[eventKey] || {listeners: []};
...
/**
 * @param {ConsolidatedAddEventListenerOptions} [eventOptions]
 * @returns {number}
 */
function eventOptionsKey(eventOptions) {
  if (!eventOptions) {
    return 0;
  }
  ...
  const capture = Number(eventOptions.capture) << 0;
  const passive = Number(eventOptions.passive) << 1;
  const once = Number(eventOptions.once) << 2;

import { convertTimezoneToLocal, dateFilter, parse } from './date';
...
describe('date', function() {
  ...
  it('should format date with default formatter', function() {
    expect(dateFilter(timestamp)).toEqual('Jan 21, 2020');
  });

  it('should format date by predefined formatter', function() {
    expect(dateFilter(timestamp, 'fullDate')).toEqual('Tuesday, January 21, 2020');
  });

  it('should format date using simple date format', function() {
    expect(dateFilter(timestamp, 'MMM/dd/yy')).toEqual('Jan/21/20');
  });

  it('should use custom timezone', function() {
    expect(dateFilter(timestamp, 'shortTime', '+04:00')).toEqual('2:00 PM');
  });

  it('should use custom timezone in GMT format', function() {
    expect(dateFilter(timestamp, 'shortTime', 'GMT+0400')).toEqual('2:00 PM');
  });

  it('should allow to pass custom formatter and locale', function() {
    expect(dateFilter(timestamp, (/**@type {Date}*/date) => `-${date.getDay()}-`)).toEqual('-2-');
  });
  ...
  describe('convertTimezoneToLocal', function() {
    it('should convert local date to the date in specific timezone', function() {
      ...
      // ⚠️
      // Get date object in the timezone +04:00 for passed date.
      // Be carefully timezone date contains invalid timestamp because under the hood we use
      // standard Date object which always in local timezone.
      // We want to use this object if we need to pass Date object
      // to a code which relay on Date object interface(.getHours, getMonth etc) to present
      // date in UI. You should not pass this object to the server
    ...
    it('should convert timezone date to the local date', function() {
      ...
      // like date picker or date input. Because this controls can present date in timezone different
      // from local so we should convert this timezone date to the local date with correct timestamp
  ...
  describe('parse', function() {
    ...
    it('should parse string in in specific timezone to the local date', () => {
      const d = parse('21/01/2020 14:00:00:000', 'd/MM/yyyy HH:mm:ss:sss', {
        timezone: '+04:00'
      });
      expect(d?.getTime()).toEqual(timestamp);
    });
  });
});
...
export function parse(dateStr, format, options) {
    ...
    const locale = (options === null || options === void 0 ? void 0 : options.dateTimeLocale) || dateTimeLocale;
    const previousDate = (options === null || options === void 0 ? void 0 : options.previousDate) || new Date(0);
    ...
    while (format && dateStr) {
        // @ts-ignore
        const match = DATE_FORMATS_SPLIT.exec(format);
        const part = match ? match[1] : format;
        format = match ? match[2] : null;
        const regexp = mappings[part] || new RegExp('\\s*' + part + '\\s*');
        const pmatch = regexp.exec(dateStr);
        if (!pmatch) {
            return null;
        }
        map[part] = pmatch[0];
        if (DATE_FORMAT_TO_STRING[part]) {
            map[part] = DATE_FORMAT_TO_STRING[part].indexOf(map[part]) + 1;
            if (!map[part]) {
                return null;
            }
        }
    ...
    map.yyyy = map.yy || map.y || map.yyyy;
    map.MM = map.MMMM || map.MMM || map.M || map.MM;
    map.dd = map.EEEE || map.EEE || map.d || map.dd;
    if (map.y || map.yy) {
        const da = new Date();
        const cent = +('' + da.getFullYear()).substr(0, 2);
        const v = parseInt(map.y !== undefined ? map.y : map.yy);
        map.yyyy = parseInt('' + (v > 68 ? cent - 1 : cent) + v);
    }
    const date = new Date(map.yyyy, map.MM - 1, map.dd, map.HH, map.mm, map.ss || 0, map.sss * 1000 || 0);
    if (map.yyyy < 100) {
...
function dateStrGetter(name, shortForm, standAlone) {
    return function (date, formats) {
        ...
        const propPrefix = (standAlone ? 'STANDALONE' : '') + (shortForm ? 'SHORT' : '');
